...
namespace FileCreateWorkerService
{
    public class Worker : BackgroundService
    {
        ...
        private async Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
            ...
            var baseUrl = "https://localhost:44390/api/files";
            using (var httpClient=new HttpClient())
            {
                var response =await httpClient.PostAsync($"{baseUrl}?fileId={cretaExcelMessage.FileId}",multipartFormDataContent);

                if (response.IsSuccessStatusCode)
                {
                    _logger.LogInformation($"File(Id :{cretaExcelMessage.FileId}) was created by successful");
