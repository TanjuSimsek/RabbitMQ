...
namespace RabbitMQWeb.WaterMarkApp.BackgroundServices
{
    public class ImageWatermarkProcessBackgroundService:BackgroundService
    {
        ...
        private Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
            ...
            try
            {
                var productCreatedEvent = JsonSerializer.Deserialize<ProductImageCreatedEvent>(Encoding.UTF8.GetString(@event.Body.ToArray()));

                var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Images", productCreatedEvent.ImageName);

                var siteNmae = "tanju.simsek@yandex.com";
                ...
                var textSize = graphic.MeasureString(siteNmae, font);
                ...
                graphic.DrawString(siteNmae, font, brush, position);
